...
class BaseExecutor:
    ...
    def __init__(
        ...
        prefetch_queries: Optional[
            Dict[str, List[Tuple[Optional[str], "QuerySet"]]]
        ] = None,
        select_related_idx: Optional[
            List[Tuple["Type[Model]", int, str, "Type[Model]"]]
        ] = None,
    ) -> None:
        self.model = model
    ...
    async def execute_select(
        self, query: Query, custom_fields: Optional[list] = None
    ) -> list:
    ...
    def get_update_sql(
        ...
        arithmetic_or_function: Optional[
            Dict[str, Union[ArithmeticExpression, Function]]
        ],
    ) -> str:
        """
        ...
        for instance in instance_list:
            if getattr(instance, relation_key_field):
                ...
                related_objects_for_fetch[key].append(
                    getattr(instance, relation_key_field)
                )
            else:
    ...
    def _make_prefetch_queries(self) -> None:
        for field_name, forwarded_prefetches in self.prefetch_map.items():
            ...
            self._prefetch_queries.setdefault(field_name, []).append(
                (to_attr, related_query)
            )
    ...
    async def _do_prefetch(
        ...
        if field in self.model._meta.backward_o2o_fields:
            return await self._prefetch_reverse_o2o_relation(
                instance_id_list, field, related_query
            )
...
class PostGISBaseExecutor(BaseExecutor):
